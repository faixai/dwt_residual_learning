...
for index_name, index_dict in final_denoised_data_dict.items(): # train and predict per each index
    ...
    for roll_num, roll_index in index_dict.items(): # train and predict with each rolled data
        ...
        # build the model
        if roll_num == 1:
            seq = Sequence(num_hidden_4, hidden_size=150, nb_layers=1)
        ...
        optimizer = optim.Adam(params=seq.parameters(), lr=0.001)
        ...
        if roll_num == 1:
            ...
            y_compounded_test_pred_lst.append(start_price)
            y_compounded_test_target_lst.append(start_price)
            date_label_lst.append(start_date)

        test_target_index = raw_test_close
        test_pred_index = utils.return_to_price(y_compounded_test_pred_lst, y_test_pred_lst)

        #temp2 = y_test.values.flatten().tolist()
        y_compounded_test_target_lst.extend(test_target_index)
        y_compounded_test_pred_lst.extend(test_pred_index)
        ...
        plt.plot(date_label, test_pred_index, 'bo-')
        plt.plot(date_label, test_target_index, 'r*-')
        ...
        plt.plot(date_label_lst, y_compounded_test_pred_lst, 'b-')
        plt.plot(date_label_lst, y_compounded_test_target_lst, 'r-')
        plt.title('{} price result 1~{} period'.format(index_name, roll_num))
