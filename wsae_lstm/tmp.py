...
for index_name, index_dict in final_denoised_data_dict.items(): # train and predict per each index
    ...
    for roll_num, roll_index in index_dict.items(): # train and predict with each rolled data
        ...
        raw_train_close = tvt_interval_data_dict[index_name][roll_num][1]['close'].copy()
        raw_val_close = tvt_interval_data_dict[index_name][roll_num][2]['close'].copy()
        raw_test_close = tvt_interval_data_dict[index_name][roll_num][3]['close'].copy()
        ...
        auto1.fit(denoised_train_x.values, denoised_val_x.values, n_epoch=n_epoch)
        ...
        data_close = pd.concat([raw_train_close, raw_val_close, raw_test_close]).reset_index(drop=True)
        ...
        # build the model
        if roll_num == 1:
            seq = Sequence(num_hidden_4, hidden_size=125, nb_layers=1)
        seq.cuda()
        ...
        for i in range(start_epoch, epochs+1):
            ...
            for j in range(len(trainloader)):
                ...
                if len(sample_x) != 0:
                    ...
                    loss_train += float(loss.data)
                    pred_train.extend(out.cpu().data.numpy().flatten().tolist())
                    target_train.extend(target.cpu().data.numpy().flatten().tolist())
            ...
            if i % 20 == 0:
                ...
                loss_val, pred_val, target_val = evaluate_lstm(dataloader=valloader, model=seq,
        ...
        loss_test, preds_test, target_test = evaluate_lstm(dataloader=testloader, model=seq,
                                                           criterion=criterion, use_gpu=use_gpu)
        ...
        '''
        x_train, y_train = prepare_data_lstm(train_encoded, y_train_input, time_steps,
                                             log_return=train_logreturn, train=False)

        trainloader = ExampleDataset(x_train, y_train, 1)

        loss_t, preds_t, target_t = evaluate_lstm(dataloader=trainloader, model=seq,
                                                  criterion=criterion, use_gpu=use_gpu)
        plt.plot(preds_t, 'bo-')
        plt.plot(target_t, 'r*-')
        plt.savefig("./plot/{}_train".format(index_name) + str(roll_num) + ".png")
        plt.show()
        '''
        # -----------------------------------------------------------------------------------------------------------
        ...
        y_test_pred = preds_test
        y_test_target = target_test
        date_label = list(final_original_data_dict[index_name][roll_num][3].copy().index)

        # Quarterly re-generate new list
        if roll_num % 4 == 1:
            y_compounded_price_pred_lst = []
            y_compounded_price_target_lst = []
            date_label_lst = []
            pred_return_list = []
            target_return_list = []
            error_list = []

            start_price = raw_val_close[-1]
            start_date = final_original_data_dict[index_name][roll_num][2].index[-1]
            y_compounded_price_pred_lst.append(start_price)
            y_compounded_price_target_lst.append(start_price)
            date_label_lst.append(start_date)

        last_price = y_compounded_price_target_lst[-1]

        test_target_index = raw_test_close
        test_pred_index = utils.return_to_price(last_price, raw_test_close, y_test_pred)

        #temp2 = y_test.values.flatten().tolist()

        y_compounded_price_target_lst.extend(test_target_index)
        y_compounded_price_pred_lst.extend(test_pred_index)
        date_label_lst.extend(date_label)

        ERROR = list(map(lambda x, y: abs((x - y) / x), test_target_index, test_pred_index))
        error_list.extend(ERROR)
        if roll_num % 4 == 0:
            MAPE = sum(error_list) / len(error_list)
            print('MAPE [index {} | roll_num {}] : '.format(index_name, roll_num), MAPE)


        # show just an 1 period return result
        plt.figure(figsize=(16, 9))
        plt.plot(date_label[:], y_test_pred, 'bo-', label='prediction')
        plt.plot(date_label[:], y_test_target, 'r*-', label='target')
        plt.title('{} return result at {} period'.format(index_name, roll_num))
        plt.legend()
        plt.grid()
        plt.savefig("./plot/{}_return".format(index_name) + str(roll_num) + ".png")
        plt.close()
        #plt.scatter(range(len(preds_test)), preds_test)
        # plt.scatter(range(len(y_test_lst)), y_test_lst)
        #plt.scatter(range(len(temp2)), temp2)

        # show just an 1 period price result
        plt.figure(figsize=(16, 9))
        plt.plot(date_label[:], test_pred_index, 'bo-', label='prediction')
        plt.plot(date_label[:], test_target_index, 'r*-', label='target')
        plt.title('{} price result at {} period'.format(index_name, roll_num))
        plt.legend()
        plt.grid()
        plt.savefig("./plot/{}_prices".format(index_name) + str(roll_num) + ".png")
        plt.close()

        # show compounded price path result so far
        plt.figure(figsize=(16, 9))
        plt.plot(date_label_lst, y_compounded_price_pred_lst, 'b-', label='prediction')
        plt.plot(date_label_lst, y_compounded_price_target_lst, 'r-', label='target')
        plt.title('{} price result {}~{} period'.format(index_name, (roll_num//4)+1, roll_num))
        plt.legend()
        plt.grid()
        plt.savefig("./plot/{}_attached_prices".format(index_name) + str(roll_num) + ".png")
        plt.close()


        # 1 period backtest result
        pred_return_list.extend(y_test_pred)
        target_return_list.extend(y_test_target)
        test_index, test_real = backtest(preds_test, y_test)

        #
        plt.figure(figsize=(16, 9))
        plt.plot(date_label[:], test_index[1:], 'bo-', label='prediction')
        plt.plot(date_label[:], test_real[1:], 'r*-', label='target')
        plt.legend()
        plt.grid()
